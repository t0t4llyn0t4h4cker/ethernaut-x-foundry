...
contract CryptoKeeperFactory is Ownable {
    ...
    function setTemplateAddress(address _templateAddress) public onlyOwner {
        require(
            ERC165Checker.supportsInterface(_templateAddress, type(ICryptoKeeper).interfaceId),
    ...
    function createCryptoKeeper(bytes32 _salt, address[] calldata _operators) external returns (address) {
    ...
    function predictCryptoKeeperAddress(bytes32 _salt) public view returns (address) {
