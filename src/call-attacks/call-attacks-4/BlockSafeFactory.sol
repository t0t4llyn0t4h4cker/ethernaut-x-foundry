...
contract BlockSafeFactory is Ownable {
    ...
    function setTemplateAddress(address _templateAddress) public onlyOwner {
        require(
            ERC165Checker.supportsInterface(_templateAddress, type(IBlockSafe).interfaceId), "Interface not supported"
        );
    ...
    function createBlockSafe(bytes32 _salt, address[] calldata _operators) external returns (address) {
    ...
    function predictBlockSafeAddress(bytes32 _salt) public view returns (address) {
        return templateAddress.predictDeterministicAddress(_salt, address(this));
    }
}
